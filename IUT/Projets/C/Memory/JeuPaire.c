...
int main(){
  int i,j,i1,i2,j1,j2,posx=0,posy=0,posx1,posx2,posy1,posy2,x,y,compteur=0,a,b,n=0,temoin=0,pause=0,cliquable=0,tour=1;
	...
	y=150;
	  ...
	  while(1){
		...
		for(j=0;j<L;j++){
		  if(((_X>=posx)&&(_Y>=posy))&&((_X<=x)&&(_Y<=y))){
		    if((tour==1)&&(tab1[i][j]==0)){
		      ...
		      posy2=posy;
		      tour=1;
		      if(tab2[i1][j1]==tab2[i2][j2]){
			tab1[i1][j1]=2;
			tab1[i2][j2]=2;
		      }
		      else{
			suivant2=Microsecondes();
			while(Microsecondes()!=suivant2+DELTA){
			  if(Microsecondes()>suivant)
			    {
			      n++;
			      DessinerScene(n);
			...
			c=CouleurParNom("black");
			ChoisirCouleurDessin(c);
			RemplirRectangle(posx1,posy1,150,150);
			RemplirRectangle(posx2,posy2,150,150);
			tab1[i1][j1]=0;
			tab1[i2][j2]=0;
		      }
		    }
	    ...
	    if(Finit(tab1)==1){
	      ...
	      c=CouleurParNom("Black");
	      ChoisirCouleurDessin(c);
	      EcrireTexte(700,700,"Bravo ! Vous avez gagne ! ",1);
	    }
	  }
}
